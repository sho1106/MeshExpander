...
```
ConservativeExpander          ← コア機能（デフォルトで全形状に使用）
  │
...
AssemblyExpander              ← マルチパートオーケストレータ
  │
  ├── デフォルト (useVoxelPartitioning=false)
  │     ファイルのメッシュ構造が部品境界 → 全部品を ConservativeExpander で膨張
  │
  └── オプション (useVoxelPartitioning=true)
        凸部品 → ConservativeExpander
        凹部品 → RobustSlicer (ボクセル分割)

RobustSlicer                  ← 凹形状向けオプション機能（明示的に有効化が必要）
  │
